...
class Assert {
  static fail(actual, expected, message) {
    throw new AssertError(message || `Failed value: ${actual}; ${expected !== undefined ? `Expect: ${expected.name || expected}` : ''}`);
  }
  ...
  static instanceOf(value, type, { message = '' } = {}) {
    if (!(value instanceof type)) {
      Assert.fail(value, type, message || `Failed instance: ${value}; Expect instance of ${type.name || type} class`);
    }
  }
  ...
  static typeOf(value, type, message) {
    if (!validTypes.includes(type)) {
      Assert.fail(
        value,
        type,
        message || `Assert.typeOf accept one of [${validTypes.map((t) => t.name)}] types. Use another method to validate "${type}"`
      );
    }

    if (type === Number && typeof value === 'number' && !Number.isNaN(value)) return;
  ...
  static arrayOf(value, of = [], { required = false, notEmpty = false, message = '' } = {}) {
    ...
    if (value && value.length && of.length && !value.every((i) => i && of.includes(i.constructor)))
      Assert.fail(value, `Array one of [${of.map((t) => t.name)}] types`, message);
  }
